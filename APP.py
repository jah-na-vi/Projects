...
def web_traffic(url):
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            rank = data.get('GlobalRank', {}).get('Rank')
            if rank is not None:
                rank = int(rank)
            else:
                print(f"No rank found for domain: {url}")
                return 0  # or any default value you prefer
        else:
            print(f"Failed to fetch data from API for domain: {url}. Status code: {response.status_code}")
            return 0  # or any default value you prefer

    except (requests.exceptions.RequestException, ValueError, KeyError) as e:
        print(f"Error fetching web traffic for domain {url}: {e}")
        return 0  # or any default value you prefer
...
# Example usage:
url = "example.com"
traffic_status = web_traffic(url)
print(f"Web traffic status for {url}: {traffic_status}")
